...
import logging
logging.getLogger("lib.packet").setLevel(logging.WARN)
logging.getLogger("lib.packet").setLevel(logging.WARN)

FLOW_IDLE_TIMEOUT = 10
FLOW_MEMORY_TIMEOUT = 60 * 5

CONTROLLER_IP = IPAddr('192.168.2.10')
CONTROLLER_MAC = EthAddr('5C:B9:01:07:43:FF')
...
class iplb(object):
    def __init__(self, connection, service_ip, servers=[]):
        ...
        self.con = connection
        self.mac = EthAddr('00:00:00:00:00:30')#EthAddr('5C:B9:01:07:43:FF')#self.con.eth_addr
        ...
        try:
            ...
            self.log = log  # Be nice to Python 2.6 (ugh)
    ...
    def _do_probe(self):
        ...
        # FOR LINKSYS TEST 1
        #r.hwsrc = EthAddr('5C:B9:01:07:43:FF')
        #r.protosrc = IPAddr('192.168.2.10')

        # FOR LINKSYS TEST 2
        r.hwsrc = self.mac#EthAddr('5C:B9:01:07:43:FF')
    ...
    def resource_algorithm(self):
        if len(self.resources) == 0:
            return self.live_servers.keys()[0]
        return min(self.resources, key=self.resources.get)

    def resource_saver(self):
        data = self.resources.values()
        for x in range(len(data)):
            filename = "server%s.txt" % str(x+1)
            myfile = open(filename, 'a')
            if os.path.getsize(filename) > 0:
                myfile.write("\n" + str(data[x]))
            else:
                myfile.write(str(data[x]))
            myfile.close()
    ...
    def _handle_PacketIn(self, event):
        ...
        ipp = packet.find('ipv4') # It's TCP.

        if udpp and ipp:
            if ipp.dstip == self.service_ip and ipp.srcip in self.live_servers:
                self.resources[ipp.srcip] = float(pickle.loads(udpp.next))
                self.resource_saver()   # Menyimpan resource kedalam file txt
        ...
        if not tcpp:
            ...
            # Not TCP, UDP and not ARP.  Don't know what to do with this.  Drop it.
            return drop()
        ...
        if ipp.srcip in self.servers:
            ...
            # log.debug("request for %s" % self.service_ip)
            ...
            if entry is None or entry.server not in self.live_servers:
                ...
                server = self.resource_algorithm()
...
def launch(ip, servers):
    ...
    from pox.proto.pong import launch as pong_launch
    pong_launch()

    # import logging
    logging.getLogger("proto.arp_responder").setLevel(logging.WARN)
    logging.getLogger("proto.pong").setLevel(logging.WARN)
